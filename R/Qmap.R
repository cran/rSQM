## Inspired by 'qmap' package
Qmap <- function(x,fobj,...){ 
  if(!any(class(fobj)=="fitQmapQUANT"))
    stop("class(fobj) should be fitQmapQUANT")  
  UseMethod("Qmap")
}

Qmap.default <- function(x,fobj,type=c("linear","tricub"),...){
  type <- match.arg(type)
...
Qmap.matrix <- function(x,fobj,...){
  ...
  xx <- sapply(hind,function(i){
    ...
    tr <- try(Qmap.default(x[,i],hf,...),silent=TRUE)
...
Qmap.data.frame <- function(x,fobj,...){
  x <- as.matrix(x)
  x <- Qmap.matrix(x,fobj,...)
  x <- as.data.frame(x)
  return(x)  
}
